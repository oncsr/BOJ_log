...
void init(int id, int s, int e, int n) {
	...
	merge(A(seg[id][n * 2], seg[id][n * 2 + 1], seg[id][n]));
	//merge(seg[id][n * 2].begin(), seg[id][n * 2].end(), seg[id][n * 2 + 1].begin(), seg[id][n * 2 + 1].end(), seg[id][n].begin);
}
...
int main() {
	...
	for (int x, y, k; Q--;) {
		cin >> x >> y >> k;
		long long s = -1000000, e = 1000000, m = (s + e + 1) >> 1;
		while (s < e) {
			if (cnt(x, y, m) < k) s = m;
			else e = m - 1;
			m = (s + e + 1) >> 1;
